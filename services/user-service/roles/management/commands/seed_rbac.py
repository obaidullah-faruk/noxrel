...
PERMISSIONS = [
    ("video", "upload"), ("video", "delete"), ("video", "publish"), ("video", "watch"),
    ("user", "view"), ("user", "update"), ("user", "ban"), ("user", "delete"),
    ("comment", "create"), ("comment", "delete"), ("comment", "moderate"),
    ("live", "start"), ("live", "stop"),
    ("admin", "access"), ("billing", "manage"),
]

ROLES = {
    "superadmin":        {"description": "Full access to everything", "is_system": True, "permissions": None},
    "admin":             {"description": "Full access except billing", "is_system": True,
                          "permissions": [p for p in PERMISSIONS if p != ("billing", "manage")]},
    "moderator":         {"description": "Content moderation", "is_system": True,
                          "permissions": [("comment", "moderate"), ("user", "ban"), ("video", "delete")]},
    "premium_subscriber":{"description": "4K streaming + comments", "is_system": True,
                          "permissions": [("video", "watch"), ("comment", "create")]},
    "basic_subscriber":  {"description": "1080p streaming + comments", "is_system": True,
                          "permissions": [("video", "watch"), ("comment", "create")]},
    "free_trial":        {"description": "480p streaming + comments", "is_system": True,
                          "permissions": [("video", "watch"), ("comment", "create")]},
    "guest":             {"description": "240p streaming only", "is_system": True,
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options) -> None:
        ...
        for role_name, cfg in ROLES.items():
            ...
            target_perms = list(perm_map.values()) if cfg["permissions"] is None else [
                perm_map[k] for k in cfg["permissions"] if k in perm_map
            ]
